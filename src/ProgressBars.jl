...
function format_amount(amount::Number, unit::AbstractString, unit_scale::Bool)
	if unit_scale
		if amount >= 1_000_000_000
			return @sprintf("%.1fG%s", amount / 1_000_000_000.0, unit)
		elseif amount >= 1_000_000
			return @sprintf("%.1fM%s", amount / 1_000_000.0, unit)
		elseif amount >= 1_000
			return @sprintf("%.1fk%s", amount / 1_000.0, unit)
		end
...
function newline_to_spaces(st, terminal_width)
	if isempty(st)
		return "", 1
	end
	...
	width_cumulator = 0
	extra_lines = 2
	for match in eachmatch(ansi_escape, st)
		c = match.match
		if length(c) > 1
			new_string *= c
		elseif c == "\n"
